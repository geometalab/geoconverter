...
def get_new_job_identifier_by_client_job_token(session_key, client_job_token):
    ...
    job_identifier = JobIdentifier.get_job_identifier_by_client_job_token(
        session_key,
        client_job_token)
...
def get_job_identifier_by_job_id(session_key, job_id):
    job_identifier = JobIdentifier.get_job_identifier_by_job_id(
        session_key,
        job_id)
    if len(job_identifier) == 1:
        return job_identifier[0]
...
def is_valid_job_id(code):
    return re.match(get_bounded_job_id_regex(), code) is not None
