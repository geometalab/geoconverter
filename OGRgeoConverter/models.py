...
class ShellParameter(models.Model):
    prefix = models.CharField(max_length=10, verbose_name='Parameter prefix', default='-', blank=True)
    parameter_name = models.CharField(max_length=100, blank=True)
    parameter_value = models.CharField(max_length=100, blank=True)
    value_quotation_marks = models.BooleanField(verbose_name='Value in quotation marks')
...
class ConversionJobFolder(models.Model):
    ...
    def get_conversion_job_folders(conversion_job):
        return ConversionJobFolder.objects.filter(conversion_job=conversion_job)
    
    @staticmethod
    def get_conversion_job_folder_by_path(conversion_job, folder_path):
        return ConversionJobFolder.objects.filter(conversion_job=conversion_job, folder_path=folder_path)
...
class ConversionJobShellParameter(ShellParameter):
    ...
    class Meta(ShellParameter.Meta):
        ...
        unique_together = (('conversion_job', 'parameter_name', 'parameter_value'),)

    @staticmethod
    def get_conversion_job_shell_parameters(conversion_job):
        return ConversionJobShellParameter.objects.filter(conversion_job=conversion_job)
...
class OgrFormatShellParameter(ShellParameter):
    ...
    class Meta(ShellParameter.Meta):
        ...
        unique_together = (('ogr_format', 'parameter_name', 'parameter_value'),)
        ordering = ('order',)
...
class GlobalOgrShellParameter(ShellParameter):
    ...
    def parameter_string(self):
        ...
        value = self.__get_value_string(self.parameter_value, self.value_quotation_marks, self.parameter_name=='')
