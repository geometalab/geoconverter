...
class Ogr2ogr:
    ...
    def __get_base_shell_command(self):
        ...
        if self.__source_srs != 0:
            shell_command.add_parameter('s_srs', 'EPSG:'  + str(self.__source_srs), '-', False)
        if self.__target_srs != 0:
            shell_command.add_parameter('t_srs', 'EPSG:'  + str(self.__target_srs), '-', False)
        
        # Sets Simplify parameter
        if self.__simplify_parameter > 0:
            shell_command.add_parameter('simplify', self.__simplify_parameter, '-', False)
