...
class JobHandler:
    ...
    def __get_conversion_job_query_set(self):
        conversion_jobs = ConversionJob.get_conversion_job(self.__job_identifier)
    ...
    def __get_conversion_job_folder_query_set(self, folder_path):
        ...
        conversion_job_folders = ConversionJobFolder.get_conversion_job_folder_by_path(conversion_job, folder_path)
    ...
    def __get_conversion_job_files_query_set(self):
        conversion_job = self.__get_conversion_job_query_set()[0]
        conversion_job_files = ConversionJobFile.get_conversion_job_files_by_conversion_job(conversion_job)
        return conversion_job_files
    
    def __get_conversion_job_files_query_set_from_file_match(self, file_match):
        conversion_job = self.__get_conversion_job_query_set()[0]
        conversion_job_files = ConversionJobFile.get_conversion_job_files_by_file_match(file_match)
        return conversion_job_files
    
    def __get_conversion_job_urls_query_set(self):
        conversion_job = self.__get_conversion_job_query_set()[0]
        conversion_job_urls = ConversionJobUrl.get_conversion_job_urls(conversion_job)
        return conversion_job_urls
    
    def __get_conversion_job_shell_parameter_query_set(self):
        conversion_job = self.__get_conversion_job_query_set()[0]
        conversion_job_shell_parameters = ConversionJobShellParameter.get_conversion_job_shell_parameters(conversion_job)
    ...
    def get_original_file_name(self, file_id):
        file_query_set = self.__get_conversion_job_files_query_set().filter(file_id=file_id)
    ...
    def add_file_match(self, source_path, file_dict, ogr_format_name, is_archive, is_valid):
        conversion_job_folder = self.__get_conversion_job_folder_query_set(source_path)[0]
    ...
    def get_free_file_id(self):
        ...
        file_id_tracker.conversion_job = self.__get_conversion_job_query_set()[0]
        file_id_tracker.save()
        return "id" + str(file_id_tracker.file_id)
    
    def remove_file(self, file_id):
        conversion_job_file = self.__get_conversion_job_files_query_set().filter(file_id=file_id)
    ...
    def get_matched_files(self, file_id):
        file_query_set = self.__get_conversion_job_files_query_set().filter(file_id=file_id)
        file_match = file_query_set[0].file_match
        matched_file_query_set = self.__get_conversion_job_files_query_set_from_file_match(file_match)
    ...
    def add_webservice_url(self, url_id, url):
        ...
        conversion_job_url.conversion_job = self.__get_conversion_job_query_set()[0]
        conversion_job_url.url_id = url_id
        conversion_job_url.url = url
        conversion_job_url.save()
        
    def get_webservice_url(self, url_id):
        url_query_set = self.__get_conversion_job_urls_query_set().filter(url_id=url_id)
    ...
    def add_shell_parameter_object(self, shell_parameter):
        self.add_shell_parameter(shell_parameter.prefix, shell_parameter.parameter_name, shell_parameter.parameter_value, shell_parameter.value_quotation_marks)
        
    def add_shell_parameter(self, prefix, parameter_name, parameter_value, value_quotation_marks):
        shell_parameter = ConversionJobShellParameter()
        shell_parameter.conversion_job = self.__get_conversion_job_query_set()[0]
