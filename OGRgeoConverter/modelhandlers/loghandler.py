...
class LogHandler:
    ...
    def __get_log_entry_query_set(self):
        log_entries = LogEntry.get_log_entry(self.__job_identifier.session_key, self.__job_identifier.job_id)
    ...
    def __get_ogr_log_entry_query_set(self):
        return OGRlogEntry.get_ogr_log_entry(self.__get_log_entry_query_set()[0])
    ...
    def set_all_files_converted(self):
        ...
        for ogr_log_entry in self.__get_ogr_log_entry_query_set():
            if not ogr_log_entry.has_output_file:
                ...
                break
        all_files_converted = all_files_converted and len(self.__get_ogr_log_entry_query_set()) > 0
    ...
    def set_has_no_error(self):
        ...
        for ogr_log_entry in self.__get_ogr_log_entry_query_set():
            if ogr_log_entry.error_message != '':
                has_no_error = False
                break
        has_no_error = has_no_error and len(self.__get_ogr_log_entry_query_set()) > 0
    ...
    def add_ogr_log_entry(self, sub_path, input_string, ogr_command, ogr_error_message, has_output_file):
        ...
        error_message_max_length = OGRlogEntry._meta.get_field('error_message').max_length
